...
static char     *pf_fF_width(char *arg, int pad_size, char *flags, char c)
{
    char    *pad;
    char    *dest;
...
static char     *pf_fF_width_sign(char *arg, int pad_size, char *flags, char c)
{
    char    *pad;
    char    *dest;
    char    *sign;
...
int pf_handle_fF(char **tmp, t_spec info, va_list a_list)
{
    char *new_tmp;
    char *pad;
    ...
    if (info.width_on == 1 && info.width > ft_strlen(*tmp) && ft_cinstr(info.flags, '0') == 0)
        *tmp = pf_fF_width(*tmp, info.width - ft_strlen(*tmp), info.flags, ' ');
    else if (info.width_on == 1 && info.width > ft_strlen(*tmp) && ft_cinstr(info.flags, '0') == 1)
        *tmp = pf_fF_width_sign(*tmp, info.width - ft_strlen(*tmp), info.flags, '0');
    return (1);
}